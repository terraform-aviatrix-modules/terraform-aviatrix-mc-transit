#!/usr/bin/env python3
"""
Consistency checker for terraform-aviatrix-mc-transit.
...
def check_variable_coverage(tf_vars, readme_vars):
    ...
    # Required variables (cloud, region, cidr, account) live in a separate table
    # and don't appear in the optional table — skip them.
    always_required = {"cloud", "region", "cidr", "account"}
    # Variables intentionally omitted from README (e.g. hidden backward-compat flags)
    intentionally_undocumented = {"legacy_transit_vpc"}
...
def main():
    print("\n=== terraform-aviatrix-mc-transit consistency check ===\n")
