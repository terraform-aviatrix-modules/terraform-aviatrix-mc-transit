...
# Release Prep Skill — terraform-aviatrix-mc-transit
...
Some examples reference other Terraform modules (e.g. `mc-spoke`, `mc-transit-peering`, `Azure/vnet/azurerm`). These should be updated to their latest compatible versions at release time.

First, find all external module sources:

```bash
grep -rn 'source\s*=' examples/ | grep -v "mc-transit" | sort -u
```
...
```bash
grep -rn 'source\s*=\|version\s*=' examples/ | grep -v "mc-transit"
```
...
Add a new section at the **very top** (below the `# terraform-aviatrix-mc-transit release notes` heading). Follow the established style — heading is `## <VERSION>` with no `v` prefix, subsections use `###` for named features.
